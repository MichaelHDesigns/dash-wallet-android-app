...
class GenerateWalletViewModel : BaseViewModel() {
    ...
    val lang = MutableLiveData<MnemonicLang>()
    ...
    private fun setType(value: Int) {
        when (value) {
            ...
            WALLET_RECOVERY -> {
                title.value = R.string.recovery_wallet
            }
        }
    }
    ...
    fun handleResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        when (requestCode) {
            REQUEST_SELECT_MNEMONIC_LANGUAGE -> {
                data?.getParcelableExtra<MnemonicLang>("lang")?.let {
                    lang.value = it
                }
            }
        }
